...
class OnboardingAnalysisService:
    ...
    def _initialize_openai(self):
        if settings.OPENAI_API_KEY:
            try:
                logger.info(f"Initializing OpenAI client for onboarding analysis with API key: {settings.OPENAI_API_KEY[:20]}...")
    ...
    def analyze_onboarding_questions(self, onboarding_answers: Dict[str, Any], user_name: str, transcription: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Fallback to mock analysis
                ...
                logger.warning("Neither OpenRouter nor OpenAI available, using mock analysis for testing")
    ...
    def _analyze_with_openrouter(self, onboarding_answers: Dict[str, Any], user_name: str, transcription: str = None) -> Dict[str, Any]:
...
{{
    ...
    "transcription": "{transcription if transcription else 'None'}"
        ...
        logger.info(f"OpenRouter onboarding analysis completed for user {user_name}: {analysis_data.get('risk_level', 'unknown')} risk")
        return analysis_data
    
    def _analyze_with_openai(self, onboarding_answers: Dict[str, Any], user_name: str, transcription: str = None) -> Dict[str, Any]:
...
{{
    ...
    "care_person_alert": "detailed message for care person"
        ...
        logger.info(f"OpenAI onboarding analysis completed for user {user_name}: {analysis_data.get('risk_level', 'unknown')} risk")
        return analysis_data
    
    def _create_mock_analysis(self, onboarding_answers: Dict[str, Any], user_name: str, transcription: str = None) -> Dict[str, Any]:
        ...
        if onboarding_answers.get('crisis_plan') in ['No, I need help creating one', 'What is a crisis plan?']:
            risk_level = "high"
            urgency_level = "high"
        ...
        if transcription:
            ...
            mock_analysis["summary"] += f" Audio transcription analysis: '{transcription[:100]}{'...' if len(transcription) > 100 else ''}'"
