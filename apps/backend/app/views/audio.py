...
from app.schemas.audio import AudioResponse, AudioTranscriptionRequest, AudioAnalysisRequest
...
async def upload_audio(
    ...
    try:
        ...
        logger.info(f"File info - filename: {file.filename}, content_type: {file.content_type}, size: {file.size}")
        ...
        if file_extension not in allowed_formats:
            ...
            raise HTTPException(status_code=400, detail=f"Invalid file format. Allowed: {', '.join(allowed_formats)}")
        ...
        logger.info(f"Audio upload completed and analysis pipeline started for audio ID: {audio.id}")
...
async def _transcribe_audio_task(audio_id: int, user_id: int, file_path: str, db: Session):
    ...
    try:
        ...
        # Update database
        audio_controller.update_transcription(db, audio_id, user_id, transcription, confidence, duration)
...
async def analyze_audio(
    ...
    try:
        ...
        if audio.analysis_status == "completed":
            return {"message": "Audio already analyzed", "analysis": {
...
async def check_openai_status(
    ...
    try:
        ...
        status = {
            ...
            "api_key_preview": settings.OPENAI_API_KEY[:20] + "..." if settings.OPENAI_API_KEY else None,
            "client_initialized": audio_controller.openai_client is not None,
            "client_type": str(type(audio_controller.openai_client)) if audio_controller.openai_client else None,
        }
...
async def _analyze_audio_task(audio_id: int, user_id: int, transcription: str, user_data: dict, db: Session):
    ...
    try:
        ...
        # Handle audio analysis failure using the presenter
        try:
            logger.info(f"Triggering onboarding analysis due to audio analysis failure for audio {audio_id}")
            from app.presenters.audio_presenter import audio_presenter
            success, message = audio_presenter.handle_analysis_failure(
                db, audio_id, user_id, user_data, str(e)
            )
            if not success:
                logger.warning(f"Onboarding analysis failed: {message}")
        except Exception as failure_handler_error:
            logger.error(f"Failed to handle audio analysis failure: {failure_handler_error}")

async def _process_audio_pipeline(audio_id: int, user_id: int, file_path: str, user_data: dict, db: Session):
    ...
    try:
        ...
        transcription, confidence, duration = audio_controller.transcribe_audio(file_path)
        logger.info(f"✅ Transcription completed for audio {audio_id} with confidence: {confidence}, duration: {duration:.2f}s")
        ...
        logger.info("=" * 80)
        audio_controller.update_transcription(db, audio_id, user_id, transcription, confidence, duration)
        logger.info(f"✅ Transcription saved to database for audio {audio_id} with duration: {duration:.2f}s")
        ...
        try:
            ...
            if immediate_alert_sent:
                logger.info(f"✅ Immediate voice alert email sent successfully for audio {audio_id}")
            else:
        ...
        logger.info("=" * 80)
        logger.info(f"📝 Transcription to analyze: \"{transcription[:100]}{'...' if len(transcription) > 100 else ''}\"")
        ...
        if audio:
            ...
            logger.info(f"📊 Final status: transcription={audio.transcription_status}, analysis={audio.analysis_status}")
        logger.info("=" * 80)
        ...
        try:
            ...
            logger.info(f"👤 User ID: {user_id}")
            logger.info(f"📧 Available emails: care_person={user_data.get('carePersonEmail')}, emergency={user_data.get('emergencyContact', {}).get('email')}")
            ...
            # Use the presenter for better error handling
            from app.presenters.audio_presenter import audio_presenter
            success, message = audio_presenter.handle_analysis_failure(
    ...
    try:
        ...
        if success:
            return {
                ...
                "emails_sent_to": [
                    email for email in [current_user.care_person_email, current_user.emergency_contact_email] 
                    if email
                ]
            }
        else:
