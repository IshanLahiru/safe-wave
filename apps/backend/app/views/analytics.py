...
async def get_user_analytics(
    ...
    try:
        ...
        current_period_audios = db.query(Audio).filter(
            and_(
        ...
        all_user_audios = db.query(Audio).filter(
            Audio.user_id == current_user.id
        ).all()
        
        # Calculate metrics
        current_total_checkins = len(current_period_audios)
        current_completed_checkins = len([a for a in current_period_audios if a.transcription_status == "completed"])
        current_completion_rate = (current_completed_checkins / current_total_checkins * 100) if current_total_checkins > 0 else 0
        ...
        if period == "week":
            for day_num in range(7):
                ...
                day_audios = [a for a in current_period_audios 
                            if day_start <= a.created_at < day_end]
                day_completed = len([a for a in day_audios if a.transcription_status == "completed"])
            ...
            for week_num in range(1, 5):
                ...
                week_audios = [a for a in current_period_audios 
                             if week_start <= a.created_at < week_end]
                week_completed = len([a for a in week_audios if a.transcription_status == "completed"])
            ...
            for month_num in range(1, 13):
                ...
                month_audios = [a for a in current_period_audios 
                              if month_start <= a.created_at < month_end]
                month_completed = len([a for a in month_audios if a.transcription_status == "completed"])
        ...
        return {
            ...
            "metrics": {
                ...
                "completion_rate": round((len([a for a in all_user_audios if a.transcription_status == "completed"]) / len(all_user_audios) * 100) if all_user_audios else 0, 1),
...
def generate_insights(audios: List[Audio], daily_pattern: Dict, risk_levels: Dict, period: str) -> List[Dict]:
