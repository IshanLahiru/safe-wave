...
export default function OnboardingQuestionnaire() {
  ...
  const handleFileUpload = async (questionId: string) => {
    try {
      ...
      if (errorObj.message.includes('cancelled') || errorObj.message.includes('aborted')) {
        ...
        errorObj.message.includes('Payload Too Large')
      ) {
        errorTitle = 'File Too Large';
        errorMessage = 'Please select a file smaller than 10MB.';
      } else if (
        errorObj.message.includes('415') ||
        errorObj.message.includes('Unsupported Media Type')
      ) {
        errorTitle = 'Unsupported File Type';
        errorMessage = 'Please select a supported file type (PDF, DOC, DOCX, JPG, PNG, TXT).';
      } else if (
        errorObj.message.includes('422') ||
        errorObj.message.includes('Unprocessable Entity')
      ) {
  ...
  const testRealUpload = async () => {
    try {
      ...
      // Create a test file
      const testContent =
        'This is a test file for upload testing. Created at: ' + new Date().toISOString();
  ...
  const testDirectUpload = async () => {
    try {
      ...
      if (Platform.OS === 'web') {
        // Web: Create Blob
        const testContent =
          'This is a test file created directly. Time: ' + new Date().toISOString();
            ...
            {!uploadedFile ? (
              <View style={styles.fileUploadOptions}>
                {fileStatus === 'uploading' ? (
                  ...
                  // Upload button
                  <TouchableOpacity
                    style={[styles.fileUploadButton, !isOnline && styles.fileUploadButtonOffline]}
                    onPress={() => handleFileUpload(currentQuestion.id)}
                    disabled={!isOnline}
                  >
                    <IconSymbol
                    ...
                    />
                    <ThemedText
                      style={[styles.fileUploadText, !isOnline && styles.fileUploadTextOffline]}
                    >
                ...
                {__DEV__ && (
                  <TouchableOpacity
                    ...
                    onPress={async () => {
                      try {
                        const { testBackendConnection, getBackendUrl } = await import(
                          '@/services/config'
                        );
                        const baseUrl = await getBackendUrl();
                        const isConnected = await testBackendConnection();
